<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<html>
<head>
	<title>일반 사용자 관리 페이지</title>
</head>
<style>
	.admin-user-card {
		width: 1000px;
		height: 300px;
		border: 1px solid black;
		display: flex;
	}
    .admin-user-img {
        width: 300px;
        max-height: 100%;
	}
	#list-area{
        width: 1800px;
		height: 600px;
		align-items: center;
		text-align: center;
		margin: auto;
	}
    .admin-user-img img {
		max-width: 200px;
		max-height: 100%;
		object-fit: cover;
		border: 1px solid red;
	}
	.admin-user-info{
        width: 700px;
		height: 100%;
        border: 1px solid blueviolet;
	}
    .memberTable {
        width: 1800px;
        margin: auto;
        text-align: center;
        border: 1px solid black;
        box-sizing: border-box;
		
    }
    .memberTable > thead {
        border-bottom: 3px solid black; /* 두 줄로 변경 */
		height: 50px;
		background-color: #2A8FF7;
		color: white;
    }
    .memberTable > thead> tr > td {
        border-right: 1px solid #0e6fcb;
	}
    .memberTable > tbody > tr {
        border-bottom: 1px solid black; /* 각 tr에 대해 선 추가 */
		height: 50px;
		color: #020715;
    }
    .memberTable > tbody > tr > td {
		border-right: 1px solid lightgray;
		box-sizing: border-box;
	}
	.pagingBtn{
		background-color: #2A8FF7;
	}
</style>

<body>
	<jsp:include page="../common/adminHeader.jsp"/>
	<script>
        function selectAllMember(cPage){
            $('.memberTable>tbody').empty();
            $('#button-area').empty();
            $.ajax({
                url: "selectAllMember.ad",
                dataType:"json",
                data: {
                    cPage: cPage
                },
                success: function (data) {
                    let memberList = data.memberList;
                    let pi = data.pi;
                    for (let i = 0; i < memberList.length; i++) {
                        let member = memberList[i];
                        console.log(member);
                        let selectMember =
                            '<tr>' +
                            '<td width="3%">'+member.userNo+'</td>' +
                            '<td width="5%">'+member.userId+'</td>' +
                            '<td width="10%">'+member.userEmail+'</td>' +
                            '<td width="4%">'+member.userName+'</td>' +
                            '<td width="5%">'+member.userNickname+'</td>' +
                            '<td width="10%">'+member.userPhone+'</td>' +
                            '<td width="5%">'+member.createDate+'</td>' +
                            '<td width="5%">'+member.lastAccessDate+'</td>' +
                            '<td width="10%">';
                        if(member.userStatus == 1){
                            selectMember +=
                                '일반 회원</td>' +
                                '<td><button class="meetgo-btn meetgo-red w-80" onclick="changeMemberStatus('+member.userNo+')">회원정지</button></td>' +
                                '</tr>';
                        } else if (member.userStatus == 2){
                            selectMember +=
                                '일반 + 고수 회원</td>' +
                                '<td><button class="meetgo-btn meetgo-red w-80" onclick="changeMemberStatus('+member.userNo+')">회원정지</button></td>' +
                                '</tr>';
                        } else if (member.userStatus == 3){
                            selectMember +=
                                '일반 회원(고수 비활성화)</td>' +
                                '<td><button class="meetgo-btn meetgo-red w-80" onclick="changeMemberStatus('+member.userNo+')">회원정지</button></td>' +
                                '</tr>';
                        } else {
                            selectMember +=
                                '회원 탈퇴</td>' +
                                '<td><button class="meetgo-btn w-80" onclick="changeMemberStatus('+member.userNo+')">정지 해제</button></td>' +
                                '</tr>';
                        }
                        $('.memberTable>tbody').append(selectMember);
                        
                    }
                    let paging = '';
                    if (pi.currentPage == 1){
                        paging += '<button class="pagingBtn" disabled style="background-color: ">이전</button>'
                    } else {
                        paging += '<button class="pagingBtn" onclick="selectAllMember('+(pi.currentPage-1)+')">이전</button>'
                    }
                    for (let i = pi.startPage; i <= pi.endPage; i++) {
                        paging += '<button class="pageBtn" onclick="selectAllMember('+i+')">' + i + '</button>'
                    }
                    if (pi.currentPage == pi.endPage){
                        paging += '<button class="pagingBtn" disabled style="background-color: ">다음</button>'
                    } else {
                        paging += '<button class="pagingBtn" onclick="selectAllMember('+(pi.currentPage+1)+')">다음</button>'
                    }
                    $('#button-area').append(paging);
                },
                error: function () {
                    console.log("실패");
                }
            })
        }
        selectAllMember(1);
        
        function changeMemberStatus(userNo){
            $.ajax({
				url: "adminMemberChangeStatus.ad",
                dataType: "json",
                method: "POST",
                data:{
                    userNo: userNo
				},
				success : function (){
    				console.log("상태 성공")
				},
				error : function (){
                    console.log("상태 실패")
				}
			})
		}
	</script>
	<div class="pageTitleArea" align="center">
		<p class="pageTitle">
			<img style="width: 40px; height: 40px"
				 src="data:image/png;base64,">
			일반 사용자 관리
		</p>
		<hr>
	</div>
	<div align="center" id="list-area">
		<table class="memberTable">
			<thead>
				<tr>
					<td width="5%">회원 번호</td>
					<td width="5%">아이디</td>
					<td width="10%">이메일</td>
					<td width="5%">이름</td>
					<td width="5%">닉네임</td>
					<td width="10%">전화번호</td>
					<td width="5%">가입일</td>
					<td width="5%">최근 접속일</td>
					<td width="5%">회원 상태</td>
					<td width="5%">정지/정지해제</td>
				</tr>
			</thead>
			<tbody>
			</tbody>
		</table>
	</div>
	<div align="center" id="button-area">
	</div>
<script>

 
</script>
</body>
</html>
